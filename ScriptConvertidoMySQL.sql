-- Convertendo script do SQL Server para MySQL
...
-- Inserção dos dados na tabela `Atores`
-- SET IDENTITY_INSERT [dbo].[Atores] ON e OFF não é necessário em MySQL
...
INSERT INTO Atores (Id, UltimoNome, Genero) VALUES (9, 'Wahlberg', 'M');
...
-- Inserção dos dados na tabela `Generos`
...
-- Inserção dos dados na tabela `ElencoFilme`
...
-- Inserção dos dados na tabela `FilmesGenero`
...
-- Adicionando Chaves Estrangeiras após a criação de todas as tabelas
ALTER TABLE ElencoFilme ADD CONSTRAINT FK_ElencoFil__IdAto__2C3393D0 FOREIGN KEY (IdAtor) REFERENCES Atores (Id);
ALTER TABLE ElencoFilme ADD CONSTRAINT FK_ElencoFil__IdFil__2D27B809 FOREIGN KEY (IdFilme) REFERENCES Filmes (Id);

ALTER TABLE FilmesGenero ADD CONSTRAINT FK_FilmesGen__IdFil__2F10007B FOREIGN KEY (IdFilme) REFERENCES Filmes (Id);
ALTER TABLE FilmesGenero ADD CONSTRAINT FK_FilmesGen__IdGen__2E1BDC42 FOREIGN KEY (IdGenero) REFERENCES Generos (Id);
